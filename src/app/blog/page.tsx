...
// import { CATEGORIES, getAllPosts, getFeaturedPost } from "@/lib/blog/posts";
...
import { DEFAULT_COVER, CATEGORIES } from "@/lib/blog/posts";
...
// আগে এটা BlogPost return করছিল (id ছিল না) — এখন UiPost return করবে (id সহ)
function toUiPost(p: DbPost): UiPost {
  ...
  return {
    ...
    category: p.category,
    date,
    readTime: minutesRead(p.content),
    excerpt: p.excerpt ?? "",
    image: p.cover_image_url ?? DEFAULT_COVER,
    tags: p.tags ?? [],
    content: p.content,
  };
}
...
export default async function BlogPage() {
  ...
  const posts = (data ?? []).map(toUiPost);
  const featured = posts[0] ?? null;

  // category safety (যদি DB থেকে ভুল category আসে)
  const safePosts = posts.filter((p) =>
    (CATEGORIES as readonly string[]).includes(p.category)
  );

  // ❌ আগে ছিল: <BlogClient featured={featured} all={safePosts} />
  // ✅ এখন BlogClient prop হলো posts
  return <BlogClient featured={featured} posts={safePosts} />;
}
